...
def format_date_range(dates):
    """Return a human-readable date range string like 'May 1 thru May 3 2026'."""
    sorted_dates = sorted(dates)
    def fmt(d):
        return d.strftime("%B %-d")  # e.g. "May 1"
    if len(sorted_dates) == 1:
        return sorted_dates[0].strftime("%B %-d %Y")
...
def process_file(file_bytes):
    ...
    company_dfs = {
        ...
        "YourTickets":  filter_company(all_df, ["YourTickets", "Your Tickets"]),
    }
    ...
    for sheet_name, cdf in company_dfs.items():
        if len(cdf) > 0:
            stats[sheet_name] = {
                "rows": len(cdf),
                "total_cost": round(float(cdf["Total Cost"].sum()), 2),
            }
